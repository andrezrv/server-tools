...
- **wps site:update** — pulls and deploys the latest WP Boilerplate code on an unmanaged site. Only runs on sites with the `unmanaged` marker; exits silently on managed sites. Called daily by the cron set up by `wps site:install`.
